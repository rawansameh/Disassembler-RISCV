...
void instDecExec(unsigned int instWord)
{
	...
	unsigned int CI_imm, CJ_imm, CL_imm, CSS_imm;

	opcode_compressed = instWord & 0x3;

	if (opcode_compressed == 0x2)
	{
		unsigned int tempCI_imm, other_rs1, tempCSS_Imm;
		...
		tempCSS_Imm = ((CSS_imm & 0x00001) << 3) | ((CSS_imm & 0x00002) << 3) | ((CSS_imm & 0x00004) << 3) | ((CSS_imm & 0x00008) >> 3) | ((CSS_imm & 0x00010) >> 3) | ((CSS_imm & 0x00020) >> 3);
		...
		switch (funct3) {
			...
			case 0: 
				cout << "\tC.SLLI\tx" << abi(rd) << ", x" << abi(rd) << ", " << hex << "0x" << (int)CI_imm << "\n";
				break;

			case 1:
				cout << "\tC.LQSP\tx" << abi(rd) << ", 0x" << (int)CI_imm << "\n";
				break;

			case 2:
				cout << "\tC.LWSP\tx" << abi(rd) << ", 0x" << (int)CI_imm << "\n";
				break;

			case 3:
				cout << "\tC.LdSP\tx" << abi(rd) << ", 0x" << (int)CI_imm << "\n";
				break;
			...
			case 6:
				cout << "\tC.SWSP\tx" << abi(rs2) << ", 0x" << (int)CSS_imm << "\n";
